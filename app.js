...
async function loadStaffTickets(search = "") {
  ...
  let rows;
  try {
    rows = await sbRpc("staff_list_tickets_manual", {
      p_username: HYPE.user,
      p_password: HYPE.pass,
      p_search: search
    });
  } catch (_) {
    rows = await sbRpc("staff_list_tickets", {
      p_username: HYPE.user,
      p_password: HYPE.pass,
      p_search: search
    });
