/* HYPE local QR renderer
 * QR encoder based on QRCode for JavaScript by Kazuhiko Arase (MIT).
 * Bundled locally so the PIX QR image does not depend on an external QR-image service.
 */
(function(global){
  'use strict';
  const modules = {};
...
module.exports = QRCode;
  ...
  function localRequire(id){
    ...
    if(!factory) throw new Error('Módulo QR local não encontrado: '+id);
  ...
  const QRErrorCorrectLevel = localRequire('./QRErrorCorrectLevel');
  ...
  function toDataUrl(text, requestedSize){
    const qr = matrix(text);
    const count = qr.getModuleCount();
    const quiet = 4;
    const target = Math.max(180, Number(requestedSize || 260));
    const cell = Math.max(1, Math.floor(target / (count + quiet * 2)));
    const size = (count + quiet * 2) * cell;
    const canvas = document.createElement('canvas');
    canvas.width = size;
    canvas.height = size;
    const ctx = canvas.getContext('2d');
    if(!ctx) throw new Error('Canvas indisponível para gerar QR Code.');
  ...
  global.HypeQRCode = { matrix, toDataUrl };
})(window);
