/* HYPE LOUNGE CLUB // PORTARIA V19 EXTRA
   - QR de pareamento renovado automaticamente (varios celulares ao mesmo tempo)
   - Lista de leitores conectados em tempo real
   - Venda na hora usando a chave PIX cadastrada no evento
   - Confirmacao do PIX limitada ao computador autorizado da Portaria
*/
(() => {
  'use strict';
  ...
  // LEITORES CONTINUOS
  // ------------------------------------------------------------------
  async function newPairQr() {
    if (state.pairBusy || !isOnline()) return;
    const key = deviceKey();
    if (!key) return;
    state.pairBusy = true;
    try {
      const token = randomSecret(24);
      const result = rows(await rpc('portaria_create_pair_v19', {
        p_device_key: key,
        p_pair_token: token
      }))[0];
      if (!result?.pairing_id) throw new Error('Não foi possível gerar o QR de conexão.');
      state.pairExpiresAt = new Date(result.expires_at).getTime();
      const url = new URL('leitor.html', location.href);
      url.searchParams.set('pair', token);
      if ($('pairQr')) $('pairQr').src = window.HypeQRCode.toDataUrl(url.toString(), 300);
      if ($('pairCode')) $('pairCode').textContent = result.pairing_code || '------';
      updatePairCountdown();
    } catch (err) {
      if ($('pairUrlText')) $('pairUrlText').textContent = err.message || 'Falha ao gerar QR.';
    } finally {
  ...
  function updatePairCountdown() {
    const el = $('pairUrlText');
    if (!el) return;
    const ms = Math.max(0, state.pairExpiresAt - Date.now());
    const sec = Math.ceil(ms / 1000);
    el.textContent = `QR temporário • renova sozinho em ${sec}s • você pode conectar vários celulares`;
    if (state.pairExpiresAt && ms <= 15000 && !state.pairBusy && $('pairModal')?.classList.contains('show')) {
      newPairQr().catch(()=>{});
    }
  }

  async function openPair() {
    if (!isOnline()) return alert('Conecte à internet para adicionar um celular leitor.');
    $('pairModal')?.classList.add('show');
    await newPairQr();
    clearInterval(state.pairCountdownTimer);
    state.pairCountdownTimer = setInterval(updatePairCountdown, 1000);
    clearInterval(state.pairTimer);
    state.pairTimer = setInterval(() => {
      if ($('pairModal')?.classList.contains('show')) newPairQr().catch(()=>{});
    }, 95000);
  }
  ...
  function closePair() {
    ...
    clearInterval(state.pairCountdownTimer); state.pairCountdownTimer = null;
  }
  ...
  async function loadReaders() {
    ...
    try {
      ...
      const grew = active.length > state.activeReaderCount;
      state.activeReaderCount = active.length;
      if (grew && $('pairModal')?.classList.contains('show')) setTimeout(()=>newPairQr().catch(()=>{}),450);
      const badge = $('readerBadge');
      if (badge) {
        badge.textContent = active.length ? `${active.length} LEITOR${active.length>1?'ES':''} CONECTADO${active.length>1?'S':''}` : 'SEM LEITOR';
        badge.className = `pill ${active.length ? 'on' : ''}`;
      }
      if (!data.length) {
        box.innerHTML = '<div class="v19-reader-empty">Nenhum celular conectado. Clique em <b>+ ADICIONAR CELULAR</b>.</div>';
        return;
      }
      box.innerHTML = data.map(r => `
        <div class="v19-reader ${r.active ? 'on' : 'off'}">
          <div class="v19-reader-main">
            ...
            <div><strong>${esc(r.reader_label || 'Celular leitor')}</strong><small>${r.active ? 'CONECTADO' : 'ENCERRADO'} • última leitura ${esc(relativeTime(r.last_scan_at))}</small></div>
          </div>
          ${r.active ? `<button class="btn red v19-reader-disconnect" onclick="HypeV19.disconnectReader('${esc(r.reader_id)}')">DESCONECTAR</button>` : ''}
        </div>`).join('');
    } catch (err) {
  ...
  async function disconnectReader(id) {
    if (!confirm('Desconectar somente este celular leitor?')) return;
    try {
      await rpc('portaria_device_disconnect_reader_v19',{p_device_key:deviceKey(),p_reader_id:id});
      await loadReaders();
    } catch (err) { alert(err.message || 'Falha ao desconectar o celular.'); }
  }

  async function endAllReaders() {
    if (!confirm('Encerrar todos os celulares leitores conectados a este computador?')) return;
    try {
  ...
  async function createDoorOrder() {
    ...
    finally { if (btn) { btn.disabled=false; btn.textContent='💠 GERAR PIX DA VENDA'; } }
  }
  ...
  function renderDoorOrder(order, paid) {
    ...
    box.innerHTML = `
      <div class="v19-order-head">
        <div><small>VENDA NA HORA</small><strong>${esc(order.customer_name||'Cliente')}</strong><span>${esc(order.ticket_code||'')}</span></div>
        <div class="v19-order-status ${isPaid?'paid':'pending'}">${isPaid?'PAGO':'AGUARDANDO PIX'}</div>
      </div>
      <div class="v19-order-grid">
        ${!isPaid ? `<div class="v19-qr-block"><h4>1. CLIENTE PAGA ESTE PIX</h4>${pix?`<img id="v19PixQr" alt="QR PIX"><textarea id="v19PixPayload" readonly>${esc(pix)}</textarea><button class="btn" onclick="HypeV19.copyPix()">COPIAR PIX COPIA E COLA</button>`:'<div class="v19-reader-empty">Chave PIX não cadastrada.</div>'}</div>` : ''}
        <div class="v19-order-info">
          ...
          ${isPaid ? '<p class="v19-paid-note">✅ Pagamento confirmado. O ingresso já está válido na Portaria.</p>' : '<p class="v19-muted">Confira o recebimento no seu banco antes de confirmar.</p>'}
        </div>
      </div>
      ${isPaid ? `<div class="v19-ticket-qr"><h4>2. QR DO INGRESSO</h4><img id="v19TicketQr" alt="QR do ingresso"><strong>${esc(order.ticket_code||'')}</strong><button class="btn green" onclick="HypeV19.showDoorTicketInPortaria()">MOSTRAR NA PORTARIA / CONFIRMAR ENTRADA</button></div>` : `<div class="v19-order-actions"><button class="btn green" onclick="HypeV19.confirmDoorPayment()">✅ CONFIRMAR PIX RECEBIDO</button><button class="btn red" onclick="HypeV19.cancelDoorOrder()">CANCELAR PEDIDO</button></div>`}
      <button class="btn v19-new-sale" onclick="HypeV19.resetDoorSale()">+ NOVA VENDA NA HORA</button>`;
  ...
  window.HypeV19 = {
    openPair, closePair, newPairQr, loadReaders, disconnectReader, endAllReaders,
