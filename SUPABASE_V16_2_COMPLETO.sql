...
begin
  ...
  if coalesce(p_id,0)=0 then
    ...
    update public.promoters_v16 p set name=trim(p_name),code=v_code,active=coalesce(p_active,true) where p.id=p_id and p.event_id=p_event_id returning p.* into v_row;
...
begin
  ...
  if coalesce(p_id,0)=0 then
    ...
    update public.coupons_v16 c set code=v_code,discount_type=p_discount_type,discount_value=p_discount_value,usage_limit=coalesce(p_usage_limit,0),starts_at=p_starts_at,ends_at=p_ends_at,active=coalesce(p_active,true)
    where c.id=p_id and c.event_id=p_event_id returning c.* into v_row;
...
-- =============================================================
-- HYPE V16.2 - COMPATIBILIDADE COM CACHE/FRONTEND V12
