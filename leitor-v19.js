/* HYPE V19 // celular somente leitor de QR
   O telefone recebe apenas uma credencial temporaria de leitor.
   Nao recebe senha, lista de clientes, valores do Admin ou acesso ao painel.
*/
(() => {
  'use strict';
  const READER_KEY='hype_reader_secret_v19';
  const LABEL_KEY='hype_reader_label_v19';
  ...
  function makeLabel(){
    ...
    if(!label){label=`Celular ${randomSecret(2).toUpperCase()}`;localStorage.setItem(LABEL_KEY,label);}
    return label;
  }

  async function claim(){
    state.label=makeLabel();
    const params=new URLSearchParams(location.search);
    const pair=params.get('pair')||'';
    if(pair){
      state.secret=randomSecret(32);
      try{
        const result=rows(await rpc('portaria_claim_pair_v19',{p_pair_token:pair,p_reader_secret:state.secret,p_reader_label:state.label}))[0];
        if(!result?.ok)throw new Error(result?.message||'QR de conexão inválido.');
        if(result.reader_label){state.label=result.reader_label;localStorage.setItem(LABEL_KEY,state.label);}
        localStorage.setItem(READER_KEY,state.secret);
        history.replaceState({},'',location.pathname);
        state.connected=true;
        setStatus(`${state.label} • CONECTADO SOMENTE COMO LEITOR`,'ok');
    ...
    const name=$('readerName');if(name)name.textContent=state.label;
    if(state.secret){state.connected=true;setStatus(`${state.label} • sessão de trabalho encontrada.`,'ok');$('startBtn').disabled=false;}
      ...
      state.timer=setInterval(async()=>{
        ...
        try{
          ...
          if(raw){
            ...
            if(ok){stop();setTimeout(()=>{if(state.connected)start();},850);}
          }
        }catch(_){}
      },300);
      setStatus(`${state.label} • LEITOR ATIVO`,'ok');
  ...
  function forget(){if(!confirm('Desconectar este celular da Portaria?'))return;stop();localStorage.removeItem(READER_KEY);state.connected=false;$('startBtn').disabled=true;setStatus('Celular desconectado. Para usar novamente, leia um novo QR no computador.','bad');}

  window.HypeReader={start,stop,sendManual,forget};
